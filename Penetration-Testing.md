# 🛡️ Pentesting Roadmap

A structured roadmap designed to guide aspiring Penetration Testers from cybersecurity fundamentals to advanced offensive security, web application testing, Active Directory exploitation, and real-world red team operations.
---

## Phase 01 — Red Hat Linux

- **Red Hat Linux (RHCSA)**
  - [Alaa Mohamed — YouTube](https://www.youtube.com/playlist?list=PLvbyo73L4vfvxbFgbbCSNNWqO1HGvrws9)

---

## Phase 02 — Web Pentesting

- **Web Development Basics** *(HTML, CSS, JS, PHP, MySQL)*
  - [Elzero Web School — HTML](https://www.youtube.com/playlist?list=PLDoPjvoNmBAw_t_XWUFbBX-c9MafPk9ji)
  - [Elzero Web School — CSS](https://www.youtube.com/playlist?list=PLDoPjvoNmBAzjsz06gkzlSrlev53MGIKe)
  - [Elzero Web School — JS](https://www.youtube.com/playlist?list=PLDoPjvoNmBAx3kiplQR_oeDqLDBUDYwVv)
  - [Elzero Web School — PHP](https://www.youtube.com/playlist?list=PLDoPjvoNmBAzH72MTPuAAaYfReraNlQgM)
  - [Elzero Web School — MySQL](https://www.youtube.com/playlist?list=PLDoPjvoNmBAz6DT8SzQ1CODJTH-NIA7R9)

- **Python**
  - [Elzero Web School](https://www.youtube.com/playlist?list=PLDoPjvoNmBAyE_gei5d18qkfIe-Z8mocs)

- **Web Pentesting**
  - [Ibrahim Hegazy](https://www.youtube.com/playlist?list=PLZNz7wrFA85CgqRMvM6c77hdckkUDigS5)

- **OWASP Top 10**
  - [Maharatech](https://maharatech.gov.eg/course/view.php?id=2144)

- **Burp Suite**
  - [Arabic Explanation — YouTube](https://youtube.com/playlist?list=PL4S940IsHJYV5gFmhh8vNqTbeUFqDktSh&si=q4PfIB3lKJZ63OZW)

- **Web Pentesting (Practical)**
  - [Arabic Playlist](https://www.youtube.com/playlist?list=PLBbacta63jciTygwsow0qBxzqBS8WhEHB)

---

## Phase 03 — Mobile Pentesting

> ابدأ بـ Java Basics الأول، وبعدين انتقل لـ Android Security.

- **Java Basics**
  - [Elzero Web School](https://youtu.be/FwxbTGxTcVI)

- **Android Pentesting**
  - [Daoud youssef](https://youtube.com/playlist?list=PL4S940IsHJYWhhYOpBk6Y-U9nTQq2omae&si=iHtByKeMw-MSk8qF)

- **OWASP Mobile Testing Guide**
  - [mas.owasp.org](https://mas.owasp.org)

- **OWASP Mobile Top 10**
  - [OWASP Egypt](https://youtu.be/fWMeUA-CsMc?si=RRSw5Mvb6UDIh4vE)

**Tools:**

- **MobSF** — [mobsf.github.io](https://github.com/mobsf/mobile-security-framework-mobsf)
- **Frida** — [frida.re](https://frida.re)
- **JADX** — [github.com/skylot/jadx](https://github.com/skylot/jadx)
- **ADB** — [developer.android.com/tools/adb](https://developer.android.com/tools/adb)
- **DVAA** — [github.com/payatu/DVAA](https://github.com/Kyd1ct/DVAA)
- **InsecureShop** — [github.com/hax0rgb/InsecureShop](https://github.com/hax0rgb/InsecureShop)

---

## Phase 04 — Network Pentesting

- **Network Pentesting**
  - [Hassan Saad](https://www.youtube.com/playlist?list=PLtr9ezc61PUbNjSOzxVn98CSy9WrJ_nUQ)

- **Active Directory Pentesting**
  - [YouTube Playlist](https://youtube.com/playlist?list=PLQbclKkFDCGNm0nAxTngZlRnCyGSLgdDM&si=EAcNvjLxowfW3Kll)

- **Nmap + Metasploit**
  - [Coursera — Arabic](https://www.coursera.org/learn/metasploit-lilmubtadiiyna-asasiaat-aliastighlal)

- **Nmap Docs**
  - [nmap.org/book](https://nmap.org/book)

- **Metasploit Docs**
  - [docs.metasploit.com](https://docs.metasploit.com)

---

## Phase 05 — OSCP Prep & Practice

> ابدأ بكورس Ahmed Atia وحل machines بالتوازي.

- **OSCP Preparation**
  - [Ahmed Atia](https://www.youtube.com/playlist?list=PLZNz7wrFA85CHJLogfqUJJUg2EkxvIa27)

- **TryHackMe** — [tryhackme.com](https://tryhackme.com)
- **Hack The Box** — [hackthebox.com](https://www.hackthebox.com)

---

Practice is what transforms knowledge into skill
